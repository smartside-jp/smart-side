...
[Claude Code 最低限セットアップ](/posts/claude-code-minimum-setup) の **装置③ ＝ 最初の1つの MCP** で、こう書きました。
...
![公開 MCP サーバー数の推移（2025 Q1 1,200 → 2026年4月 9,400+ で1年強で約8倍）](/images/posts/mcp-non-engineer-chart-1-growth.png)

- **公開 MCP サーバー数**：2025年 Q1 で 1,200 → 2026年4月で **9,400 以上**。1年強で **約8倍**（公式レジストリ集計）

![MCP SDK ダウンロード推移（公開直後ほぼゼロ → 2026年3月で月間 9,700万）](/images/posts/mcp-non-engineer-chart-2-sdk.png)

- **SDK ダウンロード数**：公開直後（2024年11月）のほぼゼロから、2026年3月で **月間 9,700万** へ。16ヶ月で爆発的に拡大
- **エンタープライズ採用**：本番運用で MCP を組み込む企業 AI チームが、複数の調査で急増中
- 大手 AI ベンダー（OpenAI / Google DeepMind / Microsoft / AWS）も対応を進めている

つまり、**個人で触れる前に、企業の現場ではもう「ある」前提** で動き始めています。

ただし、量と質は別の話です。

![公開 MCP サーバーの品質分布（高信頼の MCP は一部のみ・大半はプロトタイプや実験段階）](/images/posts/mcp-non-engineer-chart-3-quality.png)

- **「ドキュメント・保守・信頼性が揃った高信頼 MCP は、全体の一部にとどまる」** という品質調査が複数報告されている
- 大半は **プロトタイプ・実験段階・放置気味** のものが占める

「9,400 個もある」を「9,400 個から選べる」と読み替えると、**まともに動くものは限られる** という現実が裏側にあります。

最初の1つを選ぶときは、**この大半に当たらないこと** が出発点です。
...
入門者がここで挫折します。**JSON 構文ミス（カンマ抜け・括弧抜け）や再起動忘れ** が典型的な詰まりどころで、複数の解説記事が口を揃えて指摘していました。

ところが、Anthropic 公式は 2025年6月に **Desktop Extensions（当初 `.dxt`、その後 `.mcpb` にリネーム）** を発表・標準導入しました。
...
- ❌ **2025年前半まで**：JSON ファイルをテキストエディタで開いて手書き編集 → 構文ミスで詰まる
- ✅ **2025年6月以降**：Desktop Extensions でダブルクリックインストール → 動く
...
看板記事「[要らない9割を捨てる 最低限スターターキット](/posts/starter-kit-overview/)」直下の各カテゴリ記事と同じ枠組みです。
