---
title: "1Password と 2段階認証で守るベースライン ─ 数百人の事故から逆算した最初の2手"
...
看板記事「[要らない9割を捨てる 最低限スターターキット](/posts/starter-kit-overview)」の IT 編で「**3点セット**」と書いた、その正体は **1Password・2段階認証・3-2-1 バックアップ** の3つです。

この記事は、その **最初の2手（1Password と 2段階認証）** を実装レベルで残しておきます。3つ目の 3-2-1 バックアップは別記事[3-2-1 バックアップルール](/posts/backup-3-2-1-rule)で扱います。
...
**SIM スワップ詐欺**（携帯回線を乗っ取って SMS の2段階認証を奪う手口）は、英国の Cifas 統計で **2023年 289件 → 2024年 約3,000件**、なんと **1,055% 増**。

SIM スワップの怖いところは、SMS で届く認証コードを攻撃者が直接受け取るので、**「SMS の2段階認証をかけているから安心」が嘘になる** ことです。CISA と FBI は2024年12月に **SMS の2段階認証を公式に非推奨化** しました。
...
「全部入り」を捨てて、OS 標準のセキュリティ + 1Password + 2段階認証に絞ったほうが、防御層は実質的に厚くなります。
...
### 装備② ─ TOTP の2段階認証（メイン口座 + 主要 SaaS）

2段階認証は「パスワードが漏れても、2つ目の壁で止める」装置です。
...
**SMS の2段階認証は2024年12月に CISA / FBI が公式に非推奨化** しました。SIM スワップ件数が前年比 1,055% 増えた以上、銀行・証券に SMS の2段階認証しか選択肢がなければ、それは仕方ないですが、**選択肢があるなら必ず TOTP を選んでください**。
...
- **各サービスの 2段階認証のバックアップコード**：2段階認証の設定時に発行される10桁ほどの文字列を10個前後。端末紛失時にこれで2段階認証を解除できる
...
- 1Password 自体に強力な2段階認証 + Secret Key 設計があるので、金庫の強度は十分
...
**2つの装備（1Password + TOTP の2段階認証）+ 1つの補助（Emergency Kit / バックアップコードの物理保管）は変わらない。でも、組み込むサービスと優先順位は人によって違う。**

私の優先順位は「金融 > Google / Apple / Microsoft > SNS > 業務 SaaS」ですが、フリーランスで業務 SaaS が止まったら即収入ゼロ、という人なら、業務 SaaS が金融より先に来ます。

装備を持っていれば、新しいサービスを使い始めても、**自動的に「これは2段階認証 かけるべきか」を判定** できます。それが装備の最大の価値です。
...
- 2段階認証の方式も、SMS が外れたように、TOTP の次（パスキー完全移行）が来る可能性
...
- Zenn「1Password で2段階認証共有」（集約派の運用例）
